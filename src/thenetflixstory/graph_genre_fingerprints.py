...
def theme_css() -> str:
    """Small CSS helper to make Streamlit page look Netflix-dark."""
    return f"""
    <style>
    .stApp {{ background: {BG}; }}
...
def _ensure_lists(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "genres_list" not in df.columns:
        ...
        elif "genres_list" not in df.columns:
...
def compute_fingerprints(
    ...
    df = _ensure_lists(df).drop_duplicates(subset=["show_id"])
...
def create_genre_fingerprints_figure(
    ...
    # Optional titles with flags (safe in browsers)
    if use_flag_emoji:
        flags = {
